...
4. Fazer um programa que efetue a leitura dos arquivos que estão contidos no arquivo ~serie_historica_anp.rar~ [fn:1] e realize as seguintes operações (descompacte o arquivo em um diretório/pasta chamado ~serie_historica_anp~):
   ...
   - Com base na lista gerada no segundo item, deverão ser geradas as seguintes listas abaixo (cada lista deverá ser salva em formato de arquivo com os dados separados por ~;~ no diretório criado no primeiro item). O nome de cada arquivo está no início de cada tópico:
